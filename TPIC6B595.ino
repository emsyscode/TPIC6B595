...
#define TPIC6B595_in 8   // If 0 write LCD, if 1 read of LCD
#define TPIC6B595_clk 9  // if 0 is a command, if 1 is a data0
#define TPIC6B595_stb 10  // Must be pulsed to LCD fetch data of bus
...
void setup() {
    ...
    */
    ...
    DDRB =0xFF;
    PORTB =0x00;
    //only here I active the enable of interrupts to allow run the test of TPIC6B595
...
void cmd_without_stb(unsigned char a){
  ...
  //This don't send the strobe signal, to be used in burst data send
         for (mask = 0b10000000; mask>0; mask >>= 1) { //iterate through bit mask
           digitalWrite(TPIC6B595_clk, LOW);
                 if (data & mask){ // if bitwise AND resolves to true
                    digitalWrite(TPIC6B595_in, HIGH);
                 }
                 else{ //if bitwise and resolves to false
                   digitalWrite(TPIC6B595_in, LOW);
                 }
          delayMicroseconds(5);
          digitalWrite(TPIC6B595_clk, HIGH);
          delayMicroseconds(5);
         }
   digitalWrite(TPIC6B595_clk, LOW);
}
void test_AllON(void){
   digitalWrite(TPIC6B595_stb, LOW);
                          
  cmd_without_stb(0b11111111); // 
  cmd_without_stb(0b11111111); // 
                            
  digitalWrite(TPIC6B595_stb, HIGH);
  ...
  uint8_t n = 0x00;
    ...
    //
        for(uint8_t d = 0x80; d > 0; d = d >> 1){
          ...
          delay(200);  //Set to value of 2 will not showed the blink!
        }
